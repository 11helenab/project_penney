...
In general, for scoring by both tricks and cards, the optimal strategy for player 2 is to take the opposite of player 1's middle selection, and then add the first two selected from player 1's sequence to the second and third places of their selection. An example of this is when scoring by tricks, if player 1 chose the sequence "BRR", player 2 would choose "BBR" and would have an 88% chance of winning the trick. 

This method is more pronounced when scoring by cards, where there are some probabilities of player 2 winning that increase if the same sequences are chosen. For example, if player 1 chooses "RRB" and player 2 chooses "BRR", the probability player 2 wins goes from 94% when scoring by tricks to 100% when scoring by cards. The method also agrees with already published results for the game.

There are some exceptions to this strategy. When scoring by cards, if player 1 chooses the sequences "BRB" and "RBR", the rule does not hold. Instead, player 2 must take the opposite of player 1's first selection and then add the second and third places of player 1's selection. For example, if player 1 chose "BRB", player 2 should chose "RRB" and would have a 92% chance of winning. 
